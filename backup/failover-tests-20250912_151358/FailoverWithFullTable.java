...
public class FailoverWithFullTable {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String logFileName = "FAILOVER_FULL_TABLE_" + timestamp + ".log";
        logWriter = new PrintWriter(new FileWriter(logFileName));
        
        log("================================================================================");
        log("   FAILOVER TEST WITH FULL CONNECTION TABLE - BEFORE AND AFTER");
        log("================================================================================");
        log("Start time: " + sdf.format(new Date()));
        log("Log file: " + logFileName);
        log("");
        log("TEST OBJECTIVES:");
        log("  1. Show full connection table with CONNTAG BEFORE failover");
        log("  2. Trigger failover by stopping Queue Manager");
        log("  3. Show full connection table with CONNTAG AFTER failover");
        log("  4. Prove all 6 connections moved together to new QM");
        log("================================================================================\n");
        
        // Single tracking key for one connection
        String TRACKING_KEY = "FAILOVER-" + timestamp;
        
        log("🔑 TRACKING KEY: " + TRACKING_KEY);
        log("   This will track 1 parent connection + 5 sessions = 6 total");
        log("\n" + "=".repeat(80) + "\n");
        
        // ========== CONNECTION WITH 5 SESSIONS ==========
        log("╔" + "═".repeat(78) + "╗");
        log("║" + center("PHASE 1: CREATING CONNECTION WITH 5 SESSIONS", 78) + "║");
        log("╚" + "═".repeat(78) + "╝\n");
        ...
        // Print table header - adjusted for full CONNTAG display
        log(String.format("%-4s %-8s %-5s %-8s %-50s %-50s %-3s %-15s", 
            "#", "Type", "Conn", "Session", "CONNECTION_ID", "FULL_CONNTAG", "QM", "Host"));
        log("-".repeat(180));
        
        // Print all 10 connections with FULL CONNTAG values
        int rowNum = 1;
        for (ConnectionDetails details : allConnections) {
            ...
            // DO NOT TRUNCATE CONNTAG - show full value
            String fullConnTag = details.connTag != null ? details.connTag : "UNKNOWN";
            
            log(String.format("%-4d %-8s %-5s %-8s %-50s %-50s %-3s %-15s",
                rowNum++,
                details.type,
                details.connNum,
                details.sessionNum,
                connIdShort != null ? connIdShort : "UNKNOWN",
                fullConnTag,  // Full CONNTAG value, no truncation
    ...
    private static String getResolvedConnectionTag(Map<String, Object> data) {
        ...
        // COMMENTED OUT PER REQUEST - Do not compute, only use getResolvedConnectionTag() method
        /*
